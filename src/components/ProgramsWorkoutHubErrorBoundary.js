/**
 * ProgramsWorkoutHubErrorBoundary Component
 * 
 * Error boundary specifically for the consolidated Programs and Quick Workouts page.
...
class ProgramsWorkoutHubErrorBoundary extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      hasError: false,
  ...
  componentDidCatch(error, errorInfo) {
    ...
    console.error('ProgramsWorkoutHubErrorBoundary caught an error:', error, errorInfo);
    ...
    if (window.gtag) {
      window.gtag('event', 'exception', {
        ...
        custom_map: {
          ...
          view: this.props.activeView || 'unknown',
          errorBoundary: true
        }
      });
    }
  ...
  render() {
    if (this.state.hasError) {
      ...
      let errorMessage = `An unexpected error occurred while loading your ${activeView === 'programs' ? 'programs' : 'workout history'}.`;
      let showRetry = true;
      ...
      if (error?.message?.includes('Network')) {
        ...
        errorMessage = 'There was a problem accessing your data. This is usually temporary.';
      } else if (error?.message?.includes('Permission')) {
        errorTitle = 'Access Denied';
        errorMessage = 'You don\'t have permission to access this data. Please sign in again.';
        showRetry = false;
...
export default ProgramsWorkoutHubErrorBoundary;